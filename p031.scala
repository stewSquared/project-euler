...
def countChangeWays(amount: Int, coins: Int*): Int =
  if (amount < 0 || coins.length == 0) 0
  else if (amount == 0) 1
  else (countChangeWays(amount, coins.drop(1): _*) +
    countChangeWays(amount - coins(0), coins: _*))

val ans = countChangeWays(200, 200, 100, 50, 20, 10, 5, 2, 1)
