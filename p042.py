...
with open("p042.txt") as txt:
    words = sorted(eval('[' + next(txt) + ']'))

def value(word): return sum(ord(c) - 64 for c in word)

def triangular(n):
    k = int(sqrt(2*n))
    return n > 0 and k * (k + 1) / 2 == n

ans = len(list(filter(triangular, map(value, words))))
